...
function EditMeter() {
    ...
    MeterService.getById(id)
      .then((result) => {
        ...
        if (result.data.status === 'success') {
          ...
          // if (result.data.message.latitude === '' || result.data.message.longitude === '') {
  ...
  const handleSubmit = (event) => {
    ...
    if (form.checkValidity() === false) {
      ...
      MeterService.update(formData, id)
        .then((res) => {
          setValidated(false)
          setVisible(true)
          setSuccessMessage('Success: Meter updated successfully ')
          setErrorMessage('')
        })
        .catch((error) => {
    ...
    <CRow>
      <CCol xs={12}>
        ...
        <CCard className="mb-4">
          <CCardHeader>
            <strong>Meter</strong> <small>Update</small>
          </CCardHeader>
          <CCardBody>
            <CForm noValidate validated={validated} onSubmit={handleSubmit}>
              <CRow>
                ...
                <a
                  href={formData.photo_url ? formData.photo_url : ''}
                  target="_blank"
                  rel="noreferrer"
                >
                  Old Photo
